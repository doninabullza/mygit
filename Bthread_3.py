...
class Thread3(QThread):
    def __init__(self, parent):     # 메인 화면, 즉 부모의 윈도우 창을 가져오자
        ...
        self.code_in_all = None  # 계좌의 코드를 임시 저장, 쌓이지 않고 계속 갱신
        ...
        rowCount = len(self.k.BULLacc_portfolio)
        ...
        index2 = 0
        for k in self.k.BULLacc_portfolio.keys():
            self.parent.Danger_wd.setItem(index2, 0, QTableWidgetItem(str(k)))
            self.parent.Danger_wd.setItem(index2, 1, QTableWidgetItem(self.k.BULLacc_portfolio[k]["종목명"]))
            self.parent.Danger_wd.setItem(index2, 2, QTableWidgetItem(self.k.BULLacc_portfolio[k]["위험도"]))
    ...
    def C_K_F_class(self):
        ...
        code_list = []
        for code in self.k.BULLacc_portfolio.keys():
    ...
    def kigwan_meme_dong2(self, a, c):  # a. 기관일별순매수량, c. 외국인일별순매수량,
        ...
        if a[0] < 0 and a[1] < 0 and a[2] < 0 and a[3] < 0 and c[0] < 0 and c[1] < 0 and c[2] < 0 and c[3] < 0:
            self.k.BULLacc_portfolio[self.code_in_all].update({"위험도": "손절"})

        elif a[0] < 0 and a[1] < 0 and a[2] < 0 and c[0] < 0 and c[1] < 0 and c[2] < 0:
            self.k.BULLacc_portfolio[self.code_in_all].update({"위험도": "주의"})

        elif a[0] < 0 and a[1] < 0 and c[0] < 0 and c[1] < 0:
            self.k.BULLacc_portfolio[self.code_in_all].update({"위험도": "관심"})

        else:
            self.k.BULLacc_portfolio[self.code_in_all].update({"위험도": "낮음"})
