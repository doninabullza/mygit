...
class Thread1(QThread):
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "계좌평가잔고내역요청":
            ...
            self.parent.BULLstockTableWidget_1.setColumnCount(colCount)                 # 행 갯수
            self.parent.BULLstockTableWidget_1.setRowCount(rowCount)                    # 열 갯수 (종목 수)
            self.parent.BULLstockTableWidget_1.setHorizontalHeaderLabels(column_head)   # 행의 이름 삽입
            ...
            for index in range(rowCount):
                ...
                self.parent.BULLstockTableWidget_1.setItem(index, 0, QTableWidgetItem(str(itemCode)))
                self.parent.BULLstockTableWidget_1.setItem(index, 1, QTableWidgetItem(str(itemName)))
                ##천단위 구분 콤마
                self.parent.BULLstockTableWidget_1.setItem(index, 2, QTableWidgetItem(str(format(amount, ","))))
                self.parent.BULLstockTableWidget_1.setItem(index, 3, QTableWidgetItem(str(format(buyingPrice, ","))))
                self.parent.BULLstockTableWidget_1.setItem(index, 4, QTableWidgetItem(str(format(currentPrice, ","))))
                self.parent.BULLstockTableWidget_1.setItem(index, 5, QTableWidgetItem(str(format(estimateProfit, ","))))
