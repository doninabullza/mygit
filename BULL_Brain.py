...
from Bthread_1 import Thread1      # 계좌평가잔고내역 가져오기
from Bthread_2 import Thread2      # gitHub에서 오늘의 종목을 불러오는 Thread
from Bthread_3 import Thread3      # 내 계좌의 상태를 판단하자
from Bthread_4 import Thread4      # 자동매매 Thread



#=================== 프로그램 실행 하는 부분 =========================#

form_class = uic.loadUiType("BULL.ui")[0]             # 만들어 놓은 ui 불러오기

class BULL_Brain(QMainWindow, QWidget, form_class):       # QMainWindow, PyQt5에서 윈도우 생성시 필요한 함수정의

    def __init__(self, *args, **kwargs):                      # Main class의 self를 초기화 하고 시작

        print("Login Machine 실행합니다.")
        super(BULL_Brain, self).__init__(*args, **kwargs)
        form_class.__init__(self)                            # 상속 받은 from_class(BULL_Brain에서)를 실행하기 위한 초기값(초기화)
        ...
        #### git에서 txt 가지고 오기
        self.selected_url = "https://raw.githubusercontent.com/doninabullza/mygit/refs/heads/main/selected.txt"
        ...
        #####이벤트 생성 및 진행
        ### 이벤트 1. 계좌정보 가지고오기 (Bthread_1의 액션, 계좌정보 확인 버튼 연결)
        self.call_account.clicked.connect(self.B_acc)         # 계좌정보가져오기

        ###이벤트 2. 깃 허브에서 오늘의 종목 가지고오기
        self.call_selectedlist.clicked.connect(self.load_selected_list)

        ##이벤트 3. 계좌 위험도 분석 이벤트
        self.BULLacc_manager.clicked.connect(self.a_manage)
        ...
        ## 이벤트 7. 자동매매 시작/중지 버튼 연결 (이게 빠져있었습니다!)
        self.BULL_Auto_start.clicked.connect(self.auto_trade_start_stop)
    ...
    def get_account_info(self):
        ...
        for n in account_list.split(';'):
            self.BULLacc.addItem(n)

    def B_acc(self):
        print("선택 계좌 정보 가져오기")
        ##### 1번 일꾼 실행(Bthread_1 실행)
        h1 = Thread1(self)
        h1.start()
        ...
        try:
            ...
            if not self.is_auto_trading:
                ...
                if reply == QMessageBox.Yes:
                    ...
                    self.BULL_Auto_start.setText("자동매매 중지")
                    self.BULL_Auto_start.setStyleSheet("background-color: red; color: white;")
                    ...
                    try:
                        ...
                        self.is_auto_trading = False
                        self.BULL_Auto_start.setText("자동매매")
                        self.BULL_Auto_start.setStyleSheet("")
...
if __name__=='__main__':             # import된 것들을 실행시키지 않고 __main__에서 실행하는 것만 실행 시킨다.
    ...
    RS = BULL_Brain()                  # Main 클래스 myApp으로 인스턴스화
